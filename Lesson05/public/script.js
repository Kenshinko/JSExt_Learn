...
function filterWords (value) {
  const text = document.querySelector('.regexp__text');
  const regexp = new RegExp(value, 'g');
  const textR = text.innerText.replace(regexp, '"');
...
class Showcase {
  ...
  fetchGoods() {
    fetch(`${API_URL}/showcase`)
      ...
      .then((data) => {
        this.list = data.map((item) => new Goods(item));
      })
  }
  ...
  addToCart(id) {
    ...
    if (index >= 0) {
      fetch(`${API_URL}/cart`, {
        ...
        body: JSON.stringify(this.list[index])
      })
      .then((res) => {
        if (res.status === 201) {
...
class RenderGoods extends Goods {
  constructor({id, name, price}, className, sample) {
    super({id, name, price});
  ...
  renderGoods() {
    const item = `<li class='goods__item'>
                    <div class="goods__item-wrap">
                      <button class="goods__item-addtocart" onclick='showcase.addToCart(${this.id})'>
...
class CartProduct extends Cart {
  ...
  update() {
    ...
    if (currentCartTable.length < 0) {
      currentCartTable.length = 0;
    }
...
//             console.log(target);
//             const rowGoods = new Goods ({id:123, name:'Ноутбук', price:46500});
//             send((err) => console.log(err), (response) => {
//               const data = JSON.parse(response);
//               if (data.result === 1) {
//                 myCart.addGoods(rowGoods);
//               }
...
document.querySelector('.regexp__button').addEventListener('click', () => {
  const searchValue = document.querySelector('.regexp__input').value;
